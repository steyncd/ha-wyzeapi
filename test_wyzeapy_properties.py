...
async def test_zone_properties():
    ...
    if not devices:
        print("❌ No irrigation devices found!")
        return

    device = devices[0]
    print(f"\n✅ Found device: {device.nickname} ({device.mac})")
    ...
    for zone in device.zones:
        ...
        if has_is_running:
            ...
            if is_running:
                print(f"  ⚠️  ZONE IS RUNNING!")

        if has_remaining_time:
            remaining = zone.remaining_time
            print(f"  - remaining_time value: {remaining} seconds")
            if remaining > 0:
                print(f"  ⚠️  Time remaining: {remaining // 60} minutes, {remaining % 60} seconds")
    ...
    try:
        ...
        for idx, schedule in enumerate(schedules):
            ...
            if state == 'running':
                print(f"  ⚠️  RUNNING SCHEDULE FOUND!")
        ...
        print(f"❌ Error calling get_schedule_runs: {e}")
        import traceback
        traceback.print_exc()
